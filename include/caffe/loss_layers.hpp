...
/**
 * @brief Computes the  distance betweeen the predicted and
 *        ground truth.
 */
template <typename Dtype>
class DistanceMetricLayer : public Layer<Dtype> {
public:
    /**
     * @param param provides DistanceMetricParameter distance_metric_param,
     *     with DistanceMetricLayer options:
     *   - dist_m (\b optional).
     *      distance matrix for the metric
     */
    explicit DistanceMetricLayer(const LayerParameter& param)
    : Layer<Dtype>(param) {}
    virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                            const vector<Blob<Dtype>*>& top);
    virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
    ...
    virtual inline const char* type() const { return "DistanceMetric"; }
    virtual inline int ExactNumBottomBlobs() const { return 2; }
...
protected:
    ...
    /// @brief Not implemented -- DistanceMetricLayer cannot be used as a loss.
    virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
                              const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
        for (int i = 0; i < propagate_down.size(); ++i) {
            if (propagate_down[i]) { NOT_IMPLEMENTED; }
...
class WassersteinLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  /**
   * @brief Computes my loss error gradient w.r.t. the
   *        predictions.
   *
   * Gradients cannot be computed with respect to the label inputs (bottom[1]),
   * so this method ignores bottom[1] and requires !propagate_down[1], crashing
